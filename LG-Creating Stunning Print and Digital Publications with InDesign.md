...
**Learner Guide** · Version v12.0 · 19 August 2026
...
Every activity is set at **Harmony Petals**, a Singapore florist with three retail outlets. You are its junior layout designer. **Priya**, the marketing manager, briefs you; **Sun Ray Printers** produces the printed work.
...
![Figure 1.1 — The anatomy of a print page: trim, bleed, margin and slug.](courseware/assets/gen/page_anatomy.png)

*Figure 1.1 — The anatomy of a print page: trim, bleed, margin and slug.*

![Figure 1.2 — Colour mode follows the output: CMYK for ink, RGB for screen.](courseware/assets/gen/colour_modes.png)

*Figure 1.2 — Colour mode follows the output: CMYK for ink, RGB for screen.*

![Figure 1.3 — Resolution follows the medium; effective PPI is what prints.](courseware/assets/gen/resolution_ladder.png)

*Figure 1.3 — Resolution follows the medium; effective PPI is what prints.*

![Figure 1.4 — The InDesign workspace.](courseware/assets/gen/workspace_map.png)

*Figure 1.4 — The InDesign workspace.*

![Figure 1.5 — The frame and its content are two different things.](courseware/assets/gen/frame_content.png)

*Figure 1.5 — The frame and its content are two different things.*

### Activity 1 — Establish the Layout Requirement from a Client Brief
...
1. Read the brief and list every stated requirement — size, orientation, colour, stock, quantity, print process, finishing.
2. Identify the missing specifications. The brief omits bleed, margins, slug and colour mode; these must be confirmed with the printer, not assumed.
3. Look up Sun Ray Printers' specification: 3 mm bleed on all four sides, 5 mm minimum safety margin from trim, PDF/X-1a supply.
4. Compute the sizes. A5 landscape trim = 210 x 148 mm. Add 3 mm bleed on all sides for the artwork area = 216 x 154 mm.
   > `Trim 210 x 148 mm  |  Bleed 3 mm  |  Artwork 216 x 154 mm`
5. Decide the colour mode from the output: litho press = CMYK. Set image resolution at 300 ppi effective at final size.
6. Complete the Document Specification Sheet and have it checked by the trainer, acting as Priya, before proceeding.
7. Close all documents. Choose Edit (Windows) or InDesign (macOS) > Preferences > Units & Increments and set Horizontal and Vertical to Millimeters.
   > `Preferences > Units & Increments > Millimeters`
8. In Preferences > File Handling, enable Auto-activate Adobe Fonts so missing fonts are resolved automatically.
...
### Activity 2 — Create the Document and Explore the InDesign Workspace
...
1. Launch InDesign and study the Start workspace — recent files, presets, and the Adobe Stock template gallery.
2. Choose File > New > Document. Select the Print intent so the colour mode defaults to CMYK and units to millimetres.
   > `File > New > Document  ·  Intent: Print`
3. Set Width 210 mm, Height 148 mm, Orientation Landscape, Pages 2, and clear Facing Pages — this is a single-sided flyer printed both sides, not a spread.
   > `210 x 148 mm  ·  Landscape  ·  2 pages  ·  Facing Pages OFF`
4. Set all four Margins to 5 mm. Expand Bleed and Slug and set Bleed to 3 mm on all four sides.
   > `Margins 5 mm  ·  Bleed 3 mm`
5. Click Create, then File > Save As and name the file Harmony_Flyer.indd in your working folder.
   > `File > Save As  ·  Harmony_Flyer.indd`
6. Identify the red bleed line, the black trim edge and the magenta margin guide on screen. Confirm all three are visible.
7. Explore the Tools panel. Select a frame with the black Selection tool (V), then with the white Direct Selection tool (A), and observe what each one selects.
   > `V = Selection (frame)  ·  A = Direct Selection (content / points)`
8. Open Window > Links, Window > Pages, Window > Colour > Swatches and Window > Output > Preflight, and dock them together on the right.
   > `Window > Links / Pages / Swatches / Preflight`
9. Choose Window > Workspace > New Workspace, name it 'Publication Layout', tick Panel Locations and Menu Customization, and click OK.
   > `Window > Workspace > New Workspace`
10. Press W to toggle Preview mode and see the flyer as it will trim; press W again to return to Normal.
...
### Activity 3 — Build a Parent Page with Automatic Page Numbering
...
### Activity 4 — Set Up Grids, Guides and a Modular Column Structure
...
![Figure 2.1 — Threading: one story flowing through several frames.](courseware/assets/gen/threading.png)

*Figure 2.1 — Threading: one story flowing through several frames.*

![Figure 2.2 — Which Creative Cloud application owns which job.](courseware/assets/gen/app_roles.png)

*Figure 2.2 — Which Creative Cloud application owns which job.*

### Activity 5 — Create Text Frames and Import Client Copy from Word
...
### Activity 7 — Place Graphics, Manage Links and Control Effective Resolution
...
### Activity 9 — Combine Shapes with Pathfinder and Wrap Text Around a Clipping Path
...
### Activity 10 — Build a Colour System with Swatches, Spot Colours and Gradients
...
![Figure 3.1 — The styles cascade: change once, update everywhere.](courseware/assets/gen/styles_cascade.png)

*Figure 3.1 — The styles cascade: change once, update everywhere.*

![Figure 3.2 — The preflight gate before every export.](courseware/assets/gen/preflight_gate.png)

*Figure 3.2 — The preflight gate before every export.*

![Figure 3.3 — Choosing the export format by its destination.](courseware/assets/gen/export_matrix.png)

*Figure 3.3 — Choosing the export format by its destination.*

### Activity 13 — Set Type on a Path for a Curved Masthead
...
### Activity 14 — Refine Character and Paragraph Formatting for Readability
...
### Activity 16 — Import Data and Build a Formatted Price Table
...
### Activity 18 — Animate Objects with Motion Presets and Page Transitions
...
**The situation.** **Sun Ray Printers** has rejected the Harmony Petals magazine file two hours before the press booking. Their prepress report lists four faults: one image is linked to a designer's desktop and is missing, a second image has an effective resolution of 96 ppi, there is overset text in a caption frame, and two fonts are not embedded. The press slot costs S$1,150 and rolls over to next Tuesday if missed. Priya needs a clean preflight, a proper package for the printer, and a PDF/X-1a file supplied within the hour.
...
### Activity 20 — Export for Digital Delivery — Interactive PDF, EPUB and Publish Online
