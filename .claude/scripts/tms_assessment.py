...
def _api_key():
    """The LMS-TMS API key. Same resolution order as lms_push.py: shell env, then a
    project-level key file/.env, then the user-level one. Without it every LMS call
    comes back 401 'Not authenticated'."""
    import os as _os
    ...
    for path in (_os.path.join(".claude", "lms_tms_api_key"),
                 _os.path.join(home, ".claude", "lms_tms_api_key")):
        try:
...
def get_json(url):
    try:
        with urllib.request.urlopen(urllib.request.Request(url, headers=_lms_headers()), timeout=60) as r:
