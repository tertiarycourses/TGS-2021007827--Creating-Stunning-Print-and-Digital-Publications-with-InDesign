...
def main():
    ...
    # 5) Build the TOC as a REAL Word field whose CACHED RESULT is the page-numbered
    #    text below. Word/Google Docs show the cached text exactly as rendered here
    #    (so the printed PDF page numbers are correct), and the reader can still press
    #    F9 / "Update field" to regenerate it. A purely static TOC cannot refresh; a
    #    field with no cached result renders blank in headless LibreOffice. Doing both
    #    gives an accurate PDF *and* a live field.
    ...
    # field begin + instruction + separate  (all inside one paragraph)
    open_p = anchor.makeelement(qn('w:p'), {})
    anchor.addprevious(open_p)
    open_p.append(_fld('begin'))
    ri = OxmlElement('w:r')
    it = OxmlElement('w:instrText'); it.set(qn('xml:space'), 'preserve')
    it.text = f' TOC \\o "1-{maxlevel}" \\h \\z \\u '
    ri.append(it); open_p.append(ri)
    open_p.append(_fld('separate'))
    ...
    close_p = anchor.makeelement(qn('w:p'), {})
    anchor.addprevious(close_p)
    close_p.append(_fld('end'))
